...
final class BusinessDateTimeColumnConventionTest extends TestCase
{
    ...
    private const SAFE_ALLOWLIST = [
        ...
        'App\Entity\ReleasedOperatingRoomSlot::occurrenceDate' => 'date-only (Y-m-d) — Lot D, jamais un instant client à décalage horaire',
        'App\Entity\ReleasedOperatingRoomSlot::startTime' => 'time-of-day only (H:i), snapshoté depuis ShiftPeriodConfig à la création — Lot D',
        'App\Entity\ReleasedOperatingRoomSlot::endTime' => 'time-of-day only (H:i), snapshoté depuis ShiftPeriodConfig à la création — Lot D',
        ...
        'App\Entity\ReleasedOperatingRoomSlot::createdAt' => 'set from new \DateTimeImmutable() in the constructor (Lot D), never client input — not TimestampableTrait since no updatedAt is needed on an immutable, never-updated row',
