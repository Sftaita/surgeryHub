...
**Offset de `startAt`/`endAt` dans les réponses JSON (D-066)** : `Mission.startAt`/`endAt` sont mappés sur le type Doctrine sur mesure `business_datetime_immutable` (`App\Doctrine\Type\BusinessDateTimeImmutableType`) — l'objet hydraté porte déjà le vrai offset `Europe/Brussels` (`+01:00` hiver / `+02:00` été), quel que soit l'endroit du backend qui le formate. Ceci corrige structurellement tous les endroits identifiés par l'audit D-065 (`MissionMapper`, `ExportService`, `NotificationService`, `PlanningAlertService`, `InstrumentistServiceManager`, `SurgeonServiceManager`, le payload d'audit de `MissionPostDeployService::updateSchedule()`) sans modification individuelle de chacun — voir `docs/decisions.md` D-066 et `docs/architecture.md` pour la convention de stockage. **Important pour tout nouveau code qui construit `Mission.startAt`/`endAt` en interne** (générateurs de planning, scripts, etc.) : ne jamais construire un `\DateTimeImmutable` sans fuseau explicite — voir la convention dans `docs/architecture.md`.
