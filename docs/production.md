...
**Dernier tag déployé : `v2026.08.03-prod` → commit `832871e`.**
Vérifié le 2026-08-03 par marqueur de fichier réel sur le serveur (pas
seulement par le tag — voir [`docs/deployment-versioning.md`](deployment-versioning.md)
§2.2) : `FirmLogoStorage.php` présent et `Firm::$logoPath` présent dans
`Firm.php` (lot Catalogue > Prestations UX + logos, dernier des 4 lots
déployés). Pour confirmer à tout moment :

```bash
git tag -l 'v*-prod*' --sort=-creatordate | head -1
ssh surgicalhub-prod "test -f /opt/stack/apps/surgicalhub/src/backend/src/Service/FirmLogoStorage.php && echo PRESENT"
```
