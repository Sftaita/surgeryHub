...
**Dernier tag déployé : `v2026.08.02-prod-2` → commit `a27a2f4`.**
...
§2.2) : `assertRepresentativePresenceAnswered()` présent dans
`InterventionService.php` (finalisation UX instrumentiste D-092). Pour confirmer
à tout moment :

```bash
git tag -l 'v*-prod*' --sort=-creatordate | head -1
ssh surgicalhub-prod "grep -q assertRepresentativePresenceAnswered /opt/stack/apps/surgicalhub/src/backend/src/Service/InterventionService.php && echo PRESENT"
```
