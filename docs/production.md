...
**Dernier tag déployé : `v2026.08.04-prod-2` → commit `0e90469`.**
...
§2.2) : `backend/src/Enum/CatalogueRequestKind.php` présent
(lot notifications propositions catalogue). Pour confirmer à
tout moment :

```bash
git tag -l 'v*-prod*' --sort=-creatordate | head -1
ssh surgicalhub-prod "test -f /opt/stack/apps/surgicalhub/src/backend/src/Enum/CatalogueRequestKind.php && echo PRESENT"
```
