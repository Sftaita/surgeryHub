...
**Dernier tag déployé : `v2026.08.02-prod` → commit `ef97c68`.**
Vérifié le 2026-08-02 par marqueur de fichier réel sur le serveur (pas
seulement par le tag — voir [`docs/deployment-versioning.md`](deployment-versioning.md)
§2.2) : `PlanningDeployController.php` absent (retrait Planning V1),
`RepresentativePolicyResolver.php` présent. Pour confirmer à tout moment :

```bash
git tag -l 'v*-prod' --sort=-creatordate | head -1
ssh surgicalhub-prod "test -f /opt/stack/apps/surgicalhub/src/backend/src/Service/RepresentativePolicyResolver.php && echo PRESENT"
ssh surgicalhub-prod "test ! -f /opt/stack/apps/surgicalhub/src/backend/src/Controller/Api/PlanningDeployController.php && echo V1_ABSENT"
```
