...
**Dernier tag déployé : `v2026.08.05-prod` → commit `10c2505`.**
Vérifié le 2026-08-05 par test réel via l'API publique (pas seulement par le
tag — voir [`docs/deployment-versioning.md`](deployment-versioning.md) §2.2) :
`GET /api/instrumentists` expose `hasCurrentHourlyRate`, confirmé sur les 10
instrumentistes réels de prod (9 `false` / 1 `true`, cohérent avec le
diagnostic initial). Pour confirmer à tout moment :

```bash
git tag -l 'v*-prod*' --sort=-creatordate | head -1
ssh surgicalhub-prod "grep -q 'hasCurrentHourlyRate' /opt/stack/apps/surgicalhub/src/backend/src/Dto/Request/Response/InstrumentistListItemResponse.php && echo PRESENT"
```
