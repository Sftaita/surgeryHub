...
**Dernier tag déployé : `v2026.09.05-prod` → commit `763f93a`.**
Vérifié le 2026-09-05 par requêtes HTTP réelles sur le domaine public (pas
seulement par le tag — voir [`docs/deployment-versioning.md`](deployment-versioning.md)
§2.2) : login JWT réel, listing `GET` D-113 réel sur `api.surgicalhub.be`
(voir historique ci-dessous pour le détail). Pour confirmer à tout moment :
