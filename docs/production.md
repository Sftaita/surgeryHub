...
**Dernier tag déployé : `v2026.08.04-prod` → commit `75588bd`.**
...
§2.2) : `chmod($this->uploadDir, 0777)` présent dans
`FirmLogoStorage.php` (correctif permissions logo). Pour confirmer à
tout moment :

```bash
git tag -l 'v*-prod*' --sort=-creatordate | head -1
ssh surgicalhub-prod "grep -q 'chmod(\$this->uploadDir' /opt/stack/apps/surgicalhub/src/backend/src/Service/FirmLogoStorage.php && echo PRESENT"
```
