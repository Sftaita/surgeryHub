...
**Dernier tag déployé : `v2026.09.08-prod` → commit `fc19696`.**
Vérifié le 2026-09-08 par requêtes HTTP réelles sur le domaine public (pas
seulement par le tag — voir [`docs/deployment-versioning.md`](deployment-versioning.md)
§2.2) : cycle complet Générer→Brouillon→Rouvrir→Modifier→Sauvegarder→Rouvrir→
Supprimer réellement rejoué contre `api.surgicalhub.be` (voir historique
ci-dessous pour le détail). Pour confirmer à tout moment :

```bash
git tag -l 'v*-prod*' --sort=-creatordate | head -1
curl -sk -o /dev/null -w '%{http_code}\n' https://surgicalhub.be/icons/icon-32-v2.png
```
...
|---|---|---|---|
| `v2026.09.08-prod` | `fc19696` | 2026-09-08 | **CAS D (D-115) : réouverture, modification et suppression des brouillons Planning V2 persistés.** Nouveaux `GET`/`PATCH /api/planning/v2/drafts/{id}`, `DELETE /api/planning/versions/{id}` (route V1 orpheline réactivée avec garde V2), `PlanningDraftService`. Déploiement en deux temps sur ce même tag, aucun tag intermédiaire créé pour l'état partiel : (1) `04639a6` construit/déployé d'abord (build, migration `Version20260908090000` — `planning_version.preview_hash`, additive — appliquée, cache/restart, tests santé verts) ; le test fonctionnel réel en production a immédiatement révélé un `500` réel sur `DELETE /api/planning/versions/{id}` (`ForeignKeyConstraintViolationException`, `planning_alert.mission_id`, `ON DELETE RESTRICT` — une absence avait levé une `PlanningAlert SURGEON_ABSENCE` sur une Mission encore `DRAFT`) — **aucun tag créé**, conformément à la règle « un seul test critique en échec bloque le tag », état exact remonté et corrigé avant de continuer. (2) Audit complet de toutes les FK réelles vers `mission` (grep de chaque migration, croisé avec les collections `orphanRemoval` de `Mission`) : `planning_alert`/`audit_event`/`notification_event` (RESTRICT, non couvertes par `orphanRemoval`, mais légitimement peuplables sur une Mission `DRAFT`) nettoyées explicitement dans `PlanningDraftService::delete()` avant suppression des Missions, dans une transaction atomique unique ; `missing_material_report`/`mission_execution_dispute`/`encoding_anomaly_report`/`firm_invoice_line`/`instrumentist_statement_line`/`financial_calculation`/`mission_intervention_draft` (structurellement impossibles sur une vraie Mission `DRAFT`) protégées par une garde explicite (409 `PLANNING_VERSION_NOT_DRAFT` propre, jamais un `500` SQL brut, si jamais trouvées). Correctif `fc19696` committé séparément, mêmes gates repassés (backend 2277/2277, frontend 1270/1270, `tsc`/`build` verts), buildé/redéployé sur le HEAD complet — **aucune ré-exécution de la migration** (déjà appliquée par `04639a6`, confirmé par `doctrine:migrations:status`/`--dry-run` avant toute action). Correctif indépendant inclus dans le même écart : `5db5848` (erreur `tsc` préexistante et sans rapport dans `PrestationsPage.tsx`, D-111/2026-08-15, confirmée par `git blame` — jamais bloquante pour un déploiement réel puisque `docker/nginx/Dockerfile.prod` ne lance jamais `tsc -b`, corrigée par exigence de gate propre de l'utilisateur avant ce déploiement). Écart total de 4 commits (`28e4ae1` docs seules, `5db5848`, `04639a6`, `fc19696`) présenté et validé explicitement (`docs/deployment-versioning.md` §3), HEAD complet à chaque étape, jamais un sous-ensemble. Commit serveur confirmé par marqueur réel avant tout écrasement (`PlanningDraftService.php` absent avant `04639a6`, présent après ; `PROTECTED_IF_MISSION_TOUCHED` absent avant `fc19696`, présent après). Sauvegardes déploiement (première étape) : dump MySQL `/home/deploy/backups/mysql/all_20260908_101744.sql.gz` (968 Ko), archive code `/home/deploy/backups/code/src_pre_deploy_20260908_101755.tar.gz` (16 518 504 octets) — aucun nouveau backup pris pour la seconde étape (même jour, même fenêtre de déploiement, DB inchangée entre les deux). Rebuild complet `--no-cache` aux deux étapes (`BUILD_EXIT=0`), 3/3 containers recréés à chaque fois, `cache:clear` + `restart php` (worker déjà recréé par `docker compose up -d`, aucun changement de contrat Messenger dans ce lot). **Test fonctionnel réel complet en production** (`MAIL_SAFE_MODE=on` activé le temps du test, une absence réelle étant créée ; retiré et stack rechargée immédiatement après) : site + 5 comptes jetables `@surgicalhub.internal` créés (`app:user:create`), 2 postes (`BLOCK`, un avec instrumentiste, un sans) ; `POST /api/planning/v2/generate` réel → brouillon créé (2 missions) ; visible via `GET /api/planning/versions?status=DRAFT` ; second `generate()` → `409 PLANNING_DRAFT_ALREADY_EXISTS` avec l'id exact du brouillon existant ; `GET .../drafts/{id}` réel → lignes = Missions réelles persistées ; `PATCH .../drafts/{id}` réel → réaffectation enregistrée ; réouverture ultérieure confirmant la persistance (vérifiée aussi par lecture SQL directe) ; `POST /api/absences` réel sur le chirurgien d'une des deux missions → ligne redevenue `SKIPPED` à la réouverture, `divergent:true`, **Mission `DRAFT` sous-jacente intacte en base** (confirmé par lecture SQL directe, `instrumentist_id` inchangé) malgré son statut d'affichage ; `DELETE .../versions/{id}` réel → `204`, version/missions/alerte confirmés supprimés par lecture SQL directe, **absence source confirmée toujours présente** ; mois de nouveau générable (`generate()` réel → nouveau brouillon créé) ; garde non-`DRAFT` vérifiée en forçant une mission à `OPEN` par SQL direct (donnée jetable) → `DELETE` réel refusé `409 PLANNING_VERSION_NOT_DRAFT`, rien supprimé (vérifié par lecture SQL directe). **Nettoyage** : absence, missions, version, alerte, `released_operating_room_slot` (1, généré par l'absence — « Salles disponibles », indépendant), `notification_event` (2), postes, configuration de créneau, affiliations, site, 5 comptes + `refresh_tokens` — `0` résidu confirmé sur chaque table par requête de contrôle directe. Tests santé génériques (répétés après chaque étape) : frontend `200`, login `401`/`400` selon le corps envoyé (jamais `500`), `.env` `404`, 3/3 containers `Up`, migrations `Already at latest version`, logs PHP/worker sans `CRITICAL` liée au déploiement (uniquement les dépréciations Doctrine/Symfony déjà documentées sur les déploiements précédents). **Limite assumée** : le déploiement s'est déroulé en deux temps réels sur la même journée (un incident réel trouvé et corrigé entre les deux) — documenté ici comme un seul tag final couvrant l'état validé, conformément à la règle « le tag ne se pose qu'après validation complète », jamais sur un état intermédiaire. |
| `v2026.09.07-prod-2` | `f8327a1` | 2026-09-07 | **Agenda Lot D dans le planning chirurgien (`dateFrom`/`dateTo`/`period`, `GET /api/me/available-rooms/count`, calendrier mois/semaine, panneau détail jour) + récupération D-112 (garde-fou absence chirurgien sur `applyLineToMission()`, jamais `OPEN` quand le chirurgien est absent ; affichage `SKIPPED` dans l'inspecteur Planning V2 ; photos `surgeonPhotoPath`/`instrumentistPhotoPath`) + correctif indépendant `getFreedInstrumentists()` (exclusion de la ligne inspectée de sa propre liste de suggestions).** 4 commits depuis `v2026.09.07-prod` (`aca6a17`) : `4134aad` (fix backfill Lot D — bases sur les absences réelles, jamais le journal ROOM_RELEASE), `9ed1272` (agenda), `4cda4a3` (récupération D-112 — code écrit et vérifié en dev début septembre mais jamais committé jusqu'ici, confirmé absent de tout commit/branche/tag via `git log --all -S`/`git fsck --unreachable`/`git reflog`, et absent du bundle frontend déployé en prod avant ce déploiement), `f8327a1` (correctif `getFreedInstrumentists()`). Écart de 4 commits présenté et validé explicitement avant tout déploiement (`docs/deployment-versioning.md` §3, HEAD complet déployé, jamais partiel) ; commit serveur confirmé par marqueurs réels avant tout écrasement (`ReleasedOperatingRoomSlot.php` présent, `dateFrom` absent de `AvailableRoomsController.php`, `reconcileMissionAgainstCurrentAbsences` absent de `PlanningModificationService.php`, `getDrName` présent dans `RoomReleaseCommunicationService.php`) — DB (`Version20260906140000`) cohérente avec le code serveur constaté, aucune anomalie. Aucune migration dans cet écart (`New: 0` avant et après, dry-run `doctrine:migrations:migrate --dry-run` confirmé vide). Sauvegardes déploiement : dump MySQL `/home/deploy/backups/mysql/all_20260907_193402.sql.gz` (964 Ko), archive code `/home/deploy/backups/code/src_pre_deploy_20260907_193406.tar.gz` (16 499 088 octets). Rebuild complet `--no-cache` (`BUILD_EXIT=0`), 3/3 containers recréés, `cache:clear` + `restart php` + `restart worker` (consommation confirmée après redémarrage). `app:available-rooms:backfill` rejoué en prod : `--dry-run` → 24 absences analysées, 18 occurrences BLOCK futures, 18 déjà existantes, 0 seraient créées (conforme à l'état déjà backfillé antérieurement — les 18 slots réels déjà en base) ; exécution réelle → 0 créés ; rejouée une 2e fois → 0 créés (idempotence confirmée) ; `SELECT COUNT(*) FROM released_operating_room_slot` stable à 18 avant/après. **Tests fonctionnels réels en prod** (`MAIL_SAFE_MODE` non nécessaire — aucun flux email touché par cet écart) : compte chirurgien jetable `deploytest1788810043@surgicalhub.internal` créé via `app:user:create` ; login JWT réel → `200`, `/api/me` rôle `SURGEON` correct ; `GET /api/me/available-rooms/count` réel → `200`, `{"count":0}` (compte sans affiliation) ; `GET /api/me/available-rooms?dateFrom=...&dateTo=...` réel → `200`, contrat JSON correct ; `dateFrom` malformé → `400` avec message exact (jamais `500`). **Nettoyage** : `refresh_tokens` + compte de test supprimés, `0` résidu confirmé pour ce compte (un résidu **préexistant et sans rapport**, `deploytest-d111-1786863774@surgicalhub.internal` d'un déploiement antérieur — déjà signalé pour `v2026.08.16-prod` —, constaté toujours présent, non touché, hors périmètre de ce déploiement). Tests santé génériques : frontend `200`, login `400` (jamais `500`), `.env` `404`, 3/3 containers `Up`, migrations `Already at latest version`, logs PHP/worker sans `CRITICAL`/`exception` liée au déploiement (uniquement des dépréciations Doctrine/Symfony bénignes déjà documentées sur les déploiements précédents). **Limite assumée** : conformément à la demande explicite de l'utilisateur pour ce déploiement, aucun nouvel audit fonctionnel complet n'a été rejoué en prod (D-112/agenda déjà validés de façon exhaustive en local avant ces commits) — seuls les contrôles de sécurité de déploiement et les smoke tests essentiels ci-dessus ont été exécutés. |
| `v2026.09.07-prod` | `aca6a17` | 2026-09-07 | **D-114 Lot D : « Salles disponibles » — vue native SurgicalHub des créneaux BLOCK réellement libérés (jamais Google Calendar), indépendante du canal email et de `notifyColleaguesEnabled`. Inclut aussi la revue post-déploiement précédente (déplacement des contacts « gestion du bloc » vers `Hospital` + nom du chirurgien dans les emails Room Release), jamais encore déployée.** 3 commits depuis `v2026.09.06-prod` (`6f91bdc`) : `189f4b3` (docs-only, historique du déploiement précédent), `5df59b3` (revue D-114 : contacts Hospital + `getDrName()`), `aca6a17` (Lot D). Écart de 3 commits présenté et validé explicitement avant tout déploiement (`docs/deployment-versioning.md` §3, HEAD complet déployé, jamais partiel — le Lot D seul aurait été un déploiement partiel puisque `5df59b3` n'était pas encore en prod) ; commit serveur confirmé par marqueurs réels (`getDrName` absent de `RoomReleaseCommunicationService`, présent dans `BlockManagementCommunicationService` ; `ReleasedOperatingRoomSlot.php` absent) avant tout écrasement — DB (`Version20260905130000`) cohérente avec le code serveur constaté, aucune anomalie. 2 migrations additives exécutées (`Version20260906100000` : `ADD COLUMN` sur `hospital` + copie des données depuis `absence_communication_site_config` + `DROP COLUMN` des 2 champs devenus obsolètes sur cette table de configuration — jamais la table de journal, données copiées avant suppression, décision déjà documentée dans `docs/decisions.md`/`docs/architecture.md` ; `Version20260906140000` : `CREATE TABLE released_operating_room_slot`, purement additive) — dry-run relu en détail (14 requêtes) avant exécution réelle (10 requêtes, écart expliqué par le comportement transactionnel MySQL sur DDL, dépréciation Doctrine bénigne déjà rencontrée sur les déploiements précédents). Sauvegardes déploiement : dump MySQL `/home/deploy/backups/mysql/all_20260907_062950.sql.gz` (956 Ko), archive code `/home/deploy/backups/code/src_pre_deploy_20260907_062952.tar.gz` (16 467 800 octets). Rebuild complet `--no-cache` (`BUILD_EXIT=0`), 3/3 containers recréés, `cache:clear` + `restart php` après `migrate`, `restart worker` par précaution (contrat `SendTemplatedEmailMessage` inchangé dans cet écart, mais le template `absence_room_release.html.twig` modifié est rendu côté worker). **Tests fonctionnels réels en prod** (`MAIL_SAFE_MODE` non activé — aucun flux d'envoi d'email exercé par ce test, volontairement limité aux deux nouveaux endpoints de lecture) : 2 comptes jetables `@surgicalhub.internal` créés via `app:user:create` (1 manager, 1 chirurgien) ; login JWT réel → `200` pour les deux, `/api/me` rôle `SURGEON` correct ; `GET /api/planning/available-rooms` (manager) réel → `200`, `{"items":[],"page":1,"limit":25,"total":0}` (contrat JSON correct, aucune donnée préexistante) ; `GET /api/me/available-rooms` (chirurgien) réel → `200`, même contrat ; RBAC croisé vérifié dans les deux sens (`403` sur le token manager contre l'endpoint chirurgien, et inversement) ; logs PHP sans `CRITICAL`/`exception` sur la fenêtre du test. **Nettoyage** : 2 comptes de test supprimés (`DELETE FROM user`), `0` résidu confirmé. Tests santé génériques : frontend `200`, login `401` (identifiants invalides sur un essai volontairement vide, jamais `500` — les deux logins réels avec identifiants corrects ont bien renvoyé `200` comme montré ci-dessus), `.env` `404`, 3/3 containers `Up`, migrations `Already at latest version`, logs PHP/worker sans `CRITICAL`/`exception` liée au déploiement (worker : arrêt propre puis re-consommation confirmée après le `restart`, `RestartCount=0`). **Limite assumée** : le chemin d'écriture complet (création réelle d'une absence déclenchant `ReleasedOperatingRoomSlotService` + email Room Release avec le nouveau deep link) n'a pas été rejoué en prod — aurait nécessité de fabriquer un graphe complet (site, poste `BLOCK`, `ShiftPeriodConfig`, collègue affilié) pour un simple test de fumée, jugé disproportionné ; déjà validé de façon exhaustive en local juste avant ce déploiement (2245/2245 tests backend incluant le bug du deep link plain-text trouvé et corrigé en revue, voir `docs/decisions.md` D-114 Lot D) — voir ci-dessus. |
| `v2026.09.06-prod` | `6f91bdc` | 2026-09-06 | **D-114 : Communication des absences chirurgiens — Lots A+B+C déployés ensemble (« Libération de salle », « Gestion du bloc », rattrapage historique + journal manager).** 4 commits depuis `v2026.09.05-prod` (`763f93a`) : `873a31e` (docs-only, historique du déploiement précédent), `6085735` (Lot A), `d3dd382` (Lot B), `6f91bdc` (Lot C). Écart de 4 commits présenté et validé explicitement avant tout déploiement (`docs/deployment-versioning.md` §3, écart > 1 commit — HEAD complet déployé, jamais partiel) ; commit serveur confirmé par marqueur réel (`ignoreReason` présent, aucun fichier D-114 présent) avant tout écrasement. **D-112 (Planning V2, chantier séparé et toujours en cours) mis de côté par `git stash` avant la construction de l'archive** — jamais inclus dans `git archive HEAD`, restauré à l'identique juste après. 2 migrations additives exécutées (`Version20260905090000` : `absence_communication_site_config`/`surgeon_absence_communication`/`surgeon_absence_communication_delivery`, `Version20260905130000` : `dispatch_claimed_at`) — uniquement `CREATE TABLE`/`ADD COLUMN`/`ADD CONSTRAINT`/`CREATE INDEX` en `up()`, dry-run relu (14 requêtes) avant exécution réelle (10 requêtes, écart expliqué par le comportement transactionnel MySQL sur DDL — dépréciation Doctrine bénigne, sans rapport avec ce lot). Sauvegardes déploiement : dump MySQL `/home/deploy/backups/mysql/all_20260906_075447.sql.gz` (948 Ko), archive code `/home/deploy/backups/code/src_pre_deploy_20260906_075449.tar.gz` (16 359 848 octets). Rebuild complet `--no-cache` (`BUILD_EXIT=0`), 3/3 containers recréés, `cache:clear` + `restart php` après `migrate`, **`restart worker` obligatoire** (contrat `SendTemplatedEmailMessage` étendu par les Lots A/B — `cc`/`replyTo`/`absenceCommunicationDeliveryId`). **Tests fonctionnels réels en prod** (`MAIL_SAFE_MODE=on` activé le temps du test, ce lot envoyant des emails) : 3 comptes jetables `@surgicalhub.internal` créés via `app:user:create` (1 manager, 2 chirurgiens) ; login JWT réel → `200`, `/api/me` rôle correct ; site + configuration (Room Release ON, Gestion du bloc ON, délai 0j) + affiliations + un poste `BLOCK` créés en base (fixture minimale, aucun endpoint dédié) ; **`POST /api/absences` réel** (API publique, pas `docker exec`) déclenchant le vrai flux `RoomReleaseCommunicationService`+`BlockManagementCommunicationService` : 2 communications créées et réellement dispatchées via le worker Messenger réel — `MAIL_SAFE_MODE: resolved configuration` confirmé `deliveryMode=allowlist` contre le vrai relais Hostinger (`smtp.hostinger.com`, jamais `capture`), 2 emails « Libération de salle »/« Congé — » authentiquement acceptés par le relais (destinataires `@surgicalhub.internal`, dans l'allowlist), `0` erreur worker ; **`GET /api/planning/absence-communications` réel** (journal Lot C) confirme les 2 communications avec statut `SENT` correct ; **`DELETE /api/absences/{id}` réel** exercé (chemin de suppression). **Nettoyage** : absence supprimée via l'API, puis communications/deliveries/poste/affiliations/configuration/site supprimés en base, 3 comptes de test + `refresh_tokens` supprimés — `0` résidu confirmé sur toutes les tables ; `MAIL_SAFE_MODE` retiré et stack rechargée immédiatement après. Tests santé génériques : frontend `200`, login `400` (avec `Content-Type: application/json` — un premier essai sans cet en-tête a renvoyé `401`, comportement déjà documenté de cet endpoint, sans rapport avec ce lot), `.env` `404`, 3/3 containers `Up`, migrations `Already at latest version`, logs PHP/worker sans `CRITICAL`/`exception` liée au déploiement. **Revue préalable exhaustive** avant ce déploiement : 39 scénarios validés en local (API+DB+Mailpit réels) plus vérification visuelle navigateur réelle (voir `docs/decisions.md` D-114, revue finale Lot C, et le rapport de revue consolidée A+B+C). **Limite assumée** : tâche planifiée `app:absences:send-scheduled-communications` **non activée** en cron production dans ce déploiement — décision distincte et délibérée, en attente d'une validation explicite séparée (voir section « Tâche planifiée » ci-dessous, statut inchangé « PAS ENCORE ACTIVÉE »). |
| `v2026.09.05-prod` | `763f93a` | 2026-09-05 | **D-113 : correctif workflow Demandes Catalogue (synchronisation sans reload — collision de clé React Query entre le badge de nav et `CatalogueRequestsPage` corrigée —, motif d'ignore structuré + explication + verrouillage pessimiste sur `ignore()`/`resolve()`, email manager activé par défaut à la création — révision de D-094).** Inclut un correctif générique auth/router indépendant : `RequireAuth`/`LoginPage` perdaient la query string d'un deep-link lors d'une redirection à froid vers `/login` (seul `pathname` était capturé) ; capture désormais `pathname+search`, restauration exacte après authentification, `safeInternalPath()` rejette toute destination externe/protocol-relative. 2 commits depuis `v2026.08.16-prod` (`3b42560`) : `3ad1a7f` (docs-only, historique du déploiement précédent) et `763f93a` (ce lot). Écart de 2 commits présenté et validé explicitement avant tout déploiement (`docs/deployment-versioning.md` §3) — même schéma que le déploiement précédent. 1 migration additive : `Version20260904120000` (`material_item_request`/`intervention_type_request` : `ignore_reason`, `ignore_comment`, `decided_by_id` + FK, `decided_at`) — `ADD COLUMN`/`ADD CONSTRAINT`/`CREATE INDEX` uniquement en `up()`, relue avant exécution réelle, dry-run vérifié (14 requêtes), exécution réelle confirmée (12 requêtes, écart expliqué par le comportement transactionnel MySQL sur DDL — avertissement de dépréciation Doctrine bénin, déjà connu du framework, sans rapport avec ce lot). Suite complète avant commit : validation exhaustive en conditions réelles sur la stack Docker locale (nginx/PHP-FPM/MySQL/Redis/worker Messenger/Mailpit) — création demande matériel/intervention, notification manager in-app+push+email automatique via le worker réel, badge et liste synchronisés sans reload, modal Ignorer avec validation motif+explication obligatoires, email de refus avec motif exact reçu, résolution réelle, 409 sur double-traitement, 422 sur motif/explication manquants, RBAC 403 ; suite complète rejouée sans régression (auth/router : 265 tests sur 20 fichiers, tous rôles). Sauvegardes déploiement : dump MySQL `/home/deploy/backups/mysql/all_20260905_070203.sql.gz` (944 Ko), archive code `/home/deploy/backups/code/src_pre_deploy_20260905_070210.tar.gz` (16 344 288 octets). Rebuild complet `--no-cache` (`BUILD_EXIT=0`), 3/3 containers recréés, `cache:clear` + `restart php` + `restart worker` (handlers Messenger modifiés) après `migrate`. **Tests fonctionnels réels en prod** (`MAIL_SAFE_MODE=on` activé le temps du test, ce lot touchant un flux email) : compte manager jetable `deploytest-d113-1788592172@surgicalhub.internal` créé via `app:user:create` ; login JWT réel → `200`, `/api/me` rôle `MANAGER` correct ; `GET /api/material-item-requests?status=PENDING` et `GET /api/intervention-type-requests?status=PENDING` réels confirmant les 4 nouveaux champs (`ignoreReason`/`ignoreComment`/`decidedBy`/`decidedAt`, `null` sur une demande `PENDING`) ; validation `422` confirmée en conditions réelles sur `POST .../ignore` avec corps vide, sur une demande `PENDING` **réelle et pré-existante** (id 4 matériel, id 2 intervention) sans aucune mutation (statut/`ignore_reason` revérifiés inchangés après coup) — choix délibéré de ne pas fabriquer une `Mission` synthétique complète (site+chirurgien+instrumentiste) juste pour ce test, jugé disproportionné au regard du risque en production ; `404` confirmé sur un identifiant inexistant. **Nettoyage** : `MAIL_SAFE_MODE` retiré et stack rechargée immédiatement après le test ; `refresh_tokens` du compte de test supprimés puis le compte lui-même supprimé (`DELETE FROM user`, `0` résidu confirmé — aucun `AuditEvent` créé par un chemin de validation en erreur, donc pas de contrainte FK bloquante cette fois, contrairement à `v2026.08.16-prod`). Tests santé génériques : frontend `200`, login `401` (identifiants invalides, jamais `500`), `.env` `404`, 3/3 containers `Up`, migrations `Already at latest version`, logs PHP/worker sans `CRITICAL`/`exception` liée au déploiement. **Limite assumée** : le chemin de succès complet (`ignore`/`resolve` aboutissant réellement à `IGNORED`/`RESOLVED` + email de notification réel au demandeur) n'a pas été rejoué en prod avec une `Mission` réelle nouvellement créée — aurait nécessité de fabriquer un graphe complet (site, chirurgien, instrumentiste, mission) pour un simple test de fumée, jugé disproportionné ; et il était hors de question de traiter une demande `PENDING` réelle appartenant à un utilisateur réel juste pour le test. Déjà validé de façon exhaustive et équivalente en local juste avant ce déploiement (même code, mêmes handlers, image Docker construite depuis ce commit exact, worker Messenger réel, Mailpit) — voir ci-dessus. |
| `v2026.08.16-prod` | `3b42560` | 2026-08-16 | **D-111 : tarification firme conditionnée à un choix obligatoire (Signature/Altera, générique — RequiredChoiceGroup/ChoiceOption, PricingRuleResolver, exclusivité matérielle instrumentiste, confirmation de changement de choix).** 2 commits depuis `v2026.08.14-prod` (`01bf651`) : `9dbb1f4` (docs-only, historique du déploiement précédent) et `3b42560` (ce lot). Écart de 2 commits présenté et validé explicitement avant tout déploiement (`docs/deployment-versioning.md` §3). 1 migration additive : `Version20260814130000` (`required_choice_group`, `choice_option`, `pricing_rule.choice_option_id`, `mission_intervention.selected_choice_option_id`) — `CREATE TABLE`/`ADD COLUMN`/`ADD CONSTRAINT`/`CREATE INDEX` uniquement en `up()`, relue avant exécution réelle, dry-run vérifié. **Contexte** : mécanisme générique (aucune sémantique clinique en dur) permettant à un manager de configurer, pour une prestation donnée, un choix obligatoire déterminant le forfait facturé à la firme (ex. Globus TLIF : Signature 180€ vs Altera 210€) ; `PricingRuleResolver` reste l'unique moteur de résolution (invariant D-067 préservé) ; aucun montant jamais exposé à l'instrumentiste ; exclusivité matérielle appliquée côté serveur (409 `INCOMPATIBLE_CHOICE_MATERIAL`, jamais de grisement proactif dans le picker — limite documentée dans `docs/decisions.md`). Validation live complète en local avant déploiement (Chrome, dev réel) : configuration manager du groupe de choix + 2 options + forfaits discriminés, question posée à l'instrumentiste avec soumission bloquée tant que sans réponse, matériel compatible accepté, matériel incompatible rejeté en 409, dialogue de confirmation au changement de choix avec retrait effectif du matériel devenu incompatible. Suite complète avant commit : 2067/2070 tests backend, 1157/1158 tests frontend (échecs restants pré-existants, sans rapport, confirmés sur l'arbre non modifié). Sauvegardes déploiement : dump MySQL `/home/deploy/backups/mysql/all_20260816_065458.sql.gz` (920 Ko), archive code `/home/deploy/backups/code/src_pre_deploy_20260816_065508.tar.gz` (16 318 922 octets). Rebuild complet `--no-cache` (`BUILD_EXIT=0`), 3/3 containers recréés (`php`/`nginx`/`worker`), `cache:clear` + `restart php` + `restart worker` après `migrate`. **Tests fonctionnels réels en prod** (aucun flux email touché par ce lot, `MAIL_SAFE_MODE` non nécessaire) : compte manager jetable `deploytest-d111-1786863774@surgicalhub.internal` créé via `app:user:create` ; login JWT réel → `200`, `/api/me` rôle `MANAGER` correct ; firme réelle créée (`POST /api/firms` → `201`) ; type d'intervention référentiel créé (`POST /api/intervention-types` → `201`) ; prestation liée (`POST .../service-offerings` → `201`) ; groupe de choix activé (`PUT .../choice-group` → `200`, `operational:false` tant que < 2 options) ; 2 options ajoutées (`POST .../choice-group/options` → `201`×2, `operational:true` confirmé après la 2e) ; 2 `PricingRule` créées, chacune discriminée par une `ChoiceOption` distincte (`POST /api/firms/{id}/pricing-rules` avec `choiceOptionId` → `201`×2, 100€/150€) ; `GET .../pricing-rules` confirme les 2 règles correctement associées à leurs options respectives. **Nettoyage** : options + groupe de choix + prestation + type d'intervention + firme tous désactivés (`active:false`, aucun endpoint de suppression physique pour ces entités une fois une `PricingRule` réelle attachée — cohérent avec le caractère append-only de `PricingRule` depuis D-072) ; les 2 `PricingRule` de test restent en base de façon permanente par conception du système (jamais de suppression, historique financier), pas un résidu accidentel ; `refresh_tokens` du compte de test supprimés (`0` résidu confirmé) ; **le compte de test lui-même n'a pas pu être supprimé** — `DELETE FROM user` bloqué par une contrainte FK réelle (`audit_event.actor_id`, le compte a réellement créé des ressources, événement audité par conception, même limitation déjà documentée pour `v2026.08.03-prod`) ; compte identifiable sans ambiguïté par convention de nommage (`deploytest-d111-*@surgicalhub.internal`), aucune association de site, aucun rôle métier réel. Tests santé génériques : frontend `200`, login `400` (jamais `500`), `.env` `404`, 3/3 containers `Up`, migrations `Already at latest version`, logs PHP/worker sans `CRITICAL`/`exception` liée au déploiement (2 `422` bénins dans les logs, artefact d'une erreur de schéma de mon propre premier essai `pricing-rules` sans `ruleType`, corrigé immédiatement, pas une anomalie applicative). **Limite assumée** : le parcours instrumentiste complet (encodage, question obligatoire, exclusivité matérielle, confirmation de changement) n'a pas été rejoué en prod avec un compte instrumentiste réel et une vraie Mission — jugé disproportionné (aurait nécessité une affiliation de site sur un compte instrumentiste réel et la création d'une Mission réelle) au regard du risque, déjà validé de façon exhaustive et équivalente en local juste avant ce déploiement (même code, mêmes handlers, image Docker construite depuis ce commit exact) — voir ci-dessus. |
...
congé chirurgien) dont l'échéance est atteinte. **Code et migration déployés en production
depuis `v2026.09.06-prod`** (commande exécutable manuellement dès maintenant) — mais
**aucune entrée crontab n'a encore été créée** : la tâche ne s'exécute donc jamais
automatiquement au moment de la rédaction. Instructions ci-dessous prêtes pour
l'activation, à exécuter seulement avec une autorisation explicite distincte.
...
**Migration** : `Version20260905130000` (ajoute `dispatch_claimed_at` sur
`surgeon_absence_communication_delivery`, additive) **déjà appliquée en production**
(`v2026.09.06-prod`) — plus un prérequis, un fait accompli.

**Worker Messenger — déjà redémarré** lors du déploiement `v2026.09.06-prod` pour le nouveau
contrat `SendTemplatedEmailMessage` (`cc`/`replyTo`, déjà étendu une première fois en Lot A
avec `absenceCommunicationDeliveryId`) — pas d'action supplémentaire requise pour ce point
au moment d'activer le cron.
