...
14. Instrumentist account onboarding (NEW)
    14.1 Creation by manager

A manager or admin can create an instrumentist account from the manager interface.
...
initial site affiliations

Upon creation:

the user account is created with role INSTRUMENTIST

password is not set yet

the account is marked as pending profile completion

An invitation email is automatically sent.

14.2 Invitation email

The instrumentist receives an email informing them that an account has been created.

The email contains a secure onboarding link pointing to the frontend.

Example:

https://app.surgicalhub.com/onboarding/{token}
...
14.3 Profile completion flow

When opening the onboarding link, the instrumentist must complete their profile.
...
onboarding token becomes invalid

account becomes active

audit event recorded: USER_ONBOARDING_COMPLETED

14.4 Future self-registration

In a future version, instrumentists may create their own accounts.

This flow will require:

email verification

manager validation before activation

This functionality is not part of the current V1 scope.
