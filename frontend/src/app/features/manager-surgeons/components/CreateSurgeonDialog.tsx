...
function buildWarningsMessage(warnings: any[]): string {
  if (!warnings.length) {
    return "";
  }
...
export function CreateSurgeonDialog({ open, onClose }: Props) {
  ...
  const handleSubmit = async () => {
    ...
    try {
      const response = await createSurgeon({
      ...
      await queryClient.invalidateQueries({ queryKey: ["surgeons"] });

      toast.success("Chirurgien créé.");
  ...
  return (
    <Dialog
      ...
      maxWidth="sm"
    >
      <DialogTitle>Nouveau chirurgien</DialogTitle>

      <DialogContent>
        <Stack spacing={2} sx={{ mt: 1 }}>
          <Typography variant="body2" color="text.secondary">
            Création rapide d'un chirurgien.
          </Typography>
